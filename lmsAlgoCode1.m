...
function [filtered_signal, y, fc] = mlhdlc_lms_fcn(input, desired, step_size, reset_weights,order)
%
...
%s=col(input)
%display(s[2])
for i=1:1:500%for now 500

% Variable Filter
% call 'tapped_delay_fcn' function on path to create 40-step tapped delay
%delayed_signal = mtapped_delay_fcn(input(i:i+3order-1));%delayed_signal size is 1Xorder-13order-1
delayed_signal = input(i:i+order-1);
% apply filter coefficients 

%display('size of delayed_signal')

%size(delayed_signal)%its size is now 1X7order-1
weight_applied = delayed_signal .* filter_coeff;
%display('size of weight applied')
%size(weight_applied)
% call 'treesum' function on matlab path to sum up the results
filtered_signal(i) = mtreesum_fcn(weight_applied);

% Output estimated Original Signal
td = desired(i+order-1);
tf = filtered_signal(i);

esig = td - tf;
y(i) = esig;
%display('size of esig')
%size(esig)
...
%   Copyright 2007-201order-1 The MathWorks, Inc.
